...
from fusion_layer.ego_lane_estimation import estimate_ego_lane_data, estimate_ego_lane_sliding
from fusion_layer.normalize_ego_lane import normalize_ego_lane, normalize_ego_sliding
...
from line_detection_sliding.lane_detector import LaneDetector
...
def set_open_settings_callback(cb):
    """Main window meregistrasi fungsi ini agar tombol Setting bisa membuka config panel."""
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # ===============================
            # SLIDING WINDOW LANE DETECTION (FULL REPLACEMENT)
            # ===============================
            lane_frame_id += 1

            if lane_frame_id % INFER_EVERY_N_FRAMES == 0 or lane_cache is None:
                lane_data = lane_detector.process(im0_raw)
                lane_cache = lane_data

            lane_data = lane_cache

            left_fit = lane_data.get("left_fit", None)
            right_fit = lane_data.get("right_fit", None)
            fit_error = lane_data.get("fit_error", None)

            # --- INTEGRASI TRACKING PIPELINE ---
            tracking_results = pipeline.update_sliding(
                detections, 
                w=im0_raw.shape[1], 
                h=im0_raw.shape[0],
                left_fit=left_fit,
            ...
            # print("lanes_point", lanes_points)
            # print("lanes_detected", lanes_detected)

            # --------EGO LANE ESTIMATION
            ego_lane_data = estimate_ego_lane_sliding(
                left_fit, right_fit, fit_error,
            ...
            ego_lane_normalized = normalize_ego_sliding(
                left_fit=left_fit,
                right_fit=right_fit,
                frame_width=im0_raw.shape[1],
                frame_height=im0_raw.shape[0],
            ...
            im0_vis = ui.draw_lanes_sliding(im0_vis, lane_data, lane_detector.transformer)
