...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        if hasattr(self, 'btnCloseCard'):
            self.btnCloseCard.clicked.connect(self._close_center_card)
    ...
    def toggle_display(self):
        if self.btnStart.isChecked():
            ...
            last_detect_pc.disable_display()
            self.statusbar.showMessage("ADAS detection running in background...")
            self.show()
    ...
    def _load_config_from_plc(self) -> bool:
        try:
    ...
    def _refresh_alerts(self):
        ...
        if not camera_ok:
            ...
            if self.statusbar.currentMessage() != "System Ready... (PLC & Camera OK)":
                self.statusbar.showMessage("System Ready... (PLC & Camera OK)")
    ...
    def _close_camera_card(self):
        if hasattr(self, 'cardCameraAlert'):
            self.cardCameraAlert.hide()
            self.statusbar.showMessage("Camera alert ditutup.")
    ...
    def _close_center_card(self):
        if hasattr(self, 'cardConnectionAlert'):
            self.cardConnectionAlert.hide()
            self.statusbar.showMessage("Card connection alert ditutup.")
